# Carousel Element Plus API 对标计划

生成时间：2026-07-05

## 对标定位

- ElfUI 组件目录：`Data/Carousel`
- Element Plus 文档：`carousel.md`
...
- [ ] P1 补齐核心属性差距：`initial-index`、`trigger`、`indicator-position`、`arrow`、`type`、`card-scale ^`、`direction`、`motion-blur ^`、`label`
- [ ] P1 补齐事件差距：当前粗扫未发现明显缺口，进入实现时复核事件 payload 与触发时机。
- [ ] P1 补齐插槽/暴露方法：`activeIndex ^`、`setActiveItem`、`prev`、`next`
