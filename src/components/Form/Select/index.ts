...
import type { SelectFieldNames, SelectOption, SelectProps, SelectValue } from "./types";

export type { SelectFieldNames, SelectOption, SelectProps, SelectSize, SelectValue } from "./types";
...
const isEmptyValue = (value: unknown): boolean =>
  (Array.isArray(props.emptyValues) ? props.emptyValues : [undefined, null, ""]).some((item) =>
...
const viewOptionEntries = (): Array<{ option: SelectOption; index: number; key: string }> =>
...
const displayOptionEntries = (): Array<{ option: SelectOption; index: number; key: string }> =>
...
const onDropdownScroll = (event: Event): void => {
  ...
  emit("popup-scroll", { scrollTop: target.scrollTop, scrollLeft: target.scrollLeft });
...
const Select = defineHtml(html`
  ...
  >
    ...
    <span v-if=${!hasValue() && !showFilter()} class="placeholder">${props.placeholder}</span>
    ...
    <span class="suffix" part="suffix">
      <button v-if=${showClear()} type="button" class="clear" @click=${onClearClick}>×</button>
  ...
  >
    ...
    <div v-else-if=${viewOptions().length === 0} class="status">
      <slot name="empty">${filterText.value ? props.noMatchText : props.noDataText}</slot>
    </div>
    <div
    ...
    >
      <span><slot name="label">{{ optionLabel(entry.option) }}</slot></span>
