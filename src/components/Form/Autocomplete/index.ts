...
export type { AutocompleteFetchSuggestions, AutocompleteOption, AutocompleteProps } from "./types";
...
const sourceOptions = (): AutocompleteOption[] => {
  const source = suggestions.value.length ? suggestions.value : props.options || [];
...
const loadSuggestions = async (query: string): Promise<void> => {
  ...
  const result = await fetcher(query, (items) => suggestions.set(items || []));
...
const showClear = (): boolean => Boolean(props.clearable && props.modelValue && !props.disabled);
