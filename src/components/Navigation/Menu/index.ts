...
const props = defineProps({
  ...
  togglePlacement: { type: String, default: "footer" },
...
const togglePlacement = (): MenuTogglePlacement =>
  props.togglePlacement === "header" ? "header" : "footer";
...
watchEffect(() => {
  const defaults =
    !isHorizontal.value && Array.isArray(props.defaultOpeneds) ? props.defaultOpeneds : [];
  ...
  return rawItems.map((raw, fi) => {
    ...
    if (isGroup) {
      const groupIndex = String(
        item[fields.index] ?? [...ancestors, `__group_${level}_${fi}`].join("/")
      );
...
const getVisibleItems = (items = tree()): MenuViewItem[] => {
  ...
  for (const item of items) {
    ...
    if (
      !_collapsed() &&
      item.hasChildren &&
      (isOpen(item.index) || (props.searchable && searchText.value))
    )
...
const emitOpen = (item: MenuViewItem) =>
  emit("open", item.index, [...item.indexPath, item.index], item.raw);

const emitClose = (item: MenuViewItem) =>
  emit("close", item.index, [...item.indexPath, item.index], item.raw);
...
const horizontalPanelStyle = (): Record<string, string> =>
  popperStyle({ left: panelLeft.value });

const collapsePopupStyle = (): Record<string, string> =>
  popperStyle({ top: popupTop.value });

const nestedPopupStyle = (): Record<string, string> =>
  popperStyle({ top: nestedPopupTop.value });
...
const onKeydown = (e: KeyboardEvent) => {
  ...
  const items = Array.from(
    root.querySelectorAll<HTMLElement>(".menu-item:not(.is-disabled):not(.is-divider)")
  );
...
const Menu = defineHtml(html`
  <nav :class=${[ "menu" , { "is-horizontal" : isHorizontal, "is-collapsed" : isCollapsed, "is-ellipsis" :
    props.ellipsis, "is-no-collapse-transition" : !props.collapseTransition } ]} :style=${hostStyle} role="navigation"
    @keydown=${onKeydown}>
    <div class="menu-header">
      <slot name="header"></slot>
      <button v-if=${showHeaderToggle()} class="collapse-toggle collapse-toggle--header" type="button"
        @click=${toggleCollapse} :title=${toggleTitle()}>
    ...
    <template v-if=${isHorizontal}>
      <div class="horizontal-wrap">
        <div class="horizontal-bar" role="menubar">
          <button v-for="item in tree()" :key="item.index" type="button" :data-index="item.index"
            :class="['menu-item', itemClass(item)]" :disabled="item.disabled || item.divider || item.group"
            :title="item.label" role="menuitem" @click="onItemClick(item, $event)" @mouseenter="onItemEnter(item)"
        ...
        <div v-if=${getHorizontalPanelItems().length> 0}
          ...
          <template v-for="item in getHorizontalPanelItems()" :key="item.index">
            ...
            <strong v-else-if="item.group" class="menu-group-title">{{ item.label
              }}</strong>
            <button v-else type="button" :class="['menu-item', itemClass(item)]" :disabled="item.disabled"
              :title="item.label" role="menuitem" @click="onItemClick(item, $event)">
              <span v-if="item.icon" class="menu-icon">{{ item.icon }}</span>
              <span class="menu-label">{{ item.label }}</span>
              <span v-if="item.badge" class="menu-badge">{{ item.badge }}</span>
              <span v-if="item.hasChildren" class="menu-arrow">›</span>
    ...
    <template v-else>
      <div class="menu-body">
        <template v-for="item in getVisibleItems()" :key="item.index">
          ...
          <button v-else type="button" :data-index="item.index" :class="['menu-item', itemClass(item)]"
            :style="itemStyle(item)" :disabled="item.disabled" :title="isCollapsed ? item.label : ''" role="menuitem"
            @click="onItemClick(item, $event)" @mouseenter="onItemEnter(item)" @mouseleave=${onItemLeave}>
            <span v-if="item.icon" class="menu-icon">{{ item.icon }}</span>
            <span class="menu-label">{{ item.label }}</span>
            <span v-if="item.badge" class="menu-badge">{{ item.badge }}</span>
        ...
        <div v-if=${isCollapsed && getHoveredChildren().length> 0}
          ...
          >
          <button v-for="item in getHoveredChildren()" :key="item.index" type="button" :data-index="item.index"
            :class="['menu-item', itemClass(item)]" :disabled="item.disabled" :title="item.label" role="menuitem"
            @click="onItemClick(item, $event)" @mouseenter="onSubItemEnter(item)" @mouseleave=${onSubItemLeave}>
            <span v-if="item.icon" class="menu-icon">{{ item.icon }}</span>
            <span class="menu-label">{{ item.label }}</span>
            <span v-if="item.badge" class="menu-badge">{{ item.badge }}</span>
            <span v-if="item.hasChildren" class="menu-arrow">›</span>
          </button>
          <div v-if=${getHoveredChildren2().length> 0}
            :class=${nestedPopperClass()}
            role="menu"
            :style=${nestedPopupStyle()}
            >
            <button v-for="item in getHoveredChildren2()" :key="item.index" type="button" :data-index="item.index"
              :class="['menu-item', itemClass(item)]" :disabled="item.disabled" :title="item.label" role="menuitem"
      ...
      <div class="menu-footer">
        <slot name="footer"></slot>
        <button v-if=${showFooterToggle()} class="collapse-toggle" type="button" @click=${toggleCollapse}
          :title=${toggleTitle()}>
          <span class="toggle-icon">${isCollapsed ? "☰" : "✕"}</span>
        </button>
      </div>
