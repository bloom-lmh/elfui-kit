...
<elf-tag color="info">信息</elf-tag>
<elf-tag color="secondary">次要</elf-tag>`;
...
const customCode = `<elf-tag color="#7c3aed">品牌紫</elf-tag>
<elf-tag color="oklch(62% 0.2 145)" effect="plain">自定义绿色</elf-tag>`;

const staticScript = `// 静态展示案例无需额外状态。`;

const PageTagEx1 = defineHtml(html`
  <h2>颜色</h2>
  <elf-playground title="6 种语义颜色" :code=${code1} :script=${staticScript}
  ...
  <h2>变体</h2>
  <elf-playground title="filled / light / outlined" :code=${code2} :script=${staticScript}
  ...
  <h2>尺寸</h2>
  <elf-playground title="sm / md / lg" :code=${code3} :script=${staticScript}
    ><elf-tag size="sm">小</elf-tag><elf-tag size="md">中</elf-tag><elf-tag size="lg">大</elf-tag>
  </elf-playground>
  <h2>自定义颜色</h2>
  <elf-playground title="任意 CSS 颜色" :code=${customCode} :script=${staticScript}>
    <elf-tag color="#7c3aed">品牌紫</elf-tag>
    <elf-tag color="oklch(62% 0.2 145)" effect="plain">自定义绿色</elf-tag>
  </elf-playground>
`);
