import { defineHtml, html, onUnmount, useRef } from "elfui";
import { createDocsTranslator } from "../../docsLocale";

const t = createDocsTranslator({
  circle: { zh: "环形进度", en: "Circular progress" }, circleTitle: { zh: "圆形与仪表盘进度", en: "Circle and dashboard progress" },
  status: { zh: "状态与不确定进度", en: "Status and indeterminate progress" }, statusTitle: { zh: "状态、条纹与不确定进度", en: "Status, stripes, and indeterminate progress" },
  decrease: { zh: "减少", en: "Decrease" }, increase: { zh: "增加", en: "Increase" }, stop: { zh: "停止", en: "Stop" }, auto: { zh: "自动增长", en: "Auto increment" }, speed: { zh: "自动增长速度", en: "Auto-increment speed" },
  fast: { zh: "快速（200 ms）", en: "Fast (200 ms)" }, normal: { zh: "标准（400 ms）", en: "Normal (400 ms)" }, slow: { zh: "舒缓（800 ms）", en: "Relaxed (800 ms)" }
});
const circleProgress = useRef(36);
const autoRunning = useRef(false);
const autoDelay = useRef(400);
let autoTimer: number | undefined;
const speedOptions = () => [{ label: t("fast"), value: 200 }, { label: t("normal"), value: 400 }, { label: t("slow"), value: 800 }];
const clamp = (value: number): void => circleProgress.set(Math.min(100, Math.max(0, value)));
const stopAuto = (): void => { if (autoTimer !== undefined) window.clearInterval(autoTimer); autoTimer = undefined; autoRunning.set(false); };
const startAuto = (): void => { stopAuto(); if (circleProgress.value >= 100) circleProgress.set(0); autoRunning.set(true); autoTimer = window.setInterval(() => { clamp(circleProgress.value + 4); if (circleProgress.value >= 100) stopAuto(); }, autoDelay.value); };
...
const onSpeedChange = (event: CustomEvent): void => { const running = autoRunning.value; autoDelay.set(Number(event.detail) || 400); if (running) startAuto(); };
const transition = (): number => typeof window !== "undefined" && window.matchMedia?.("(prefers-reduced-motion: reduce)").matches ? 0 : 0.45;
onUnmount(stopAuto);
...
const PageProgressEx2 = defineHtml(html`
  ...
  <elf-playground :title=${t("circleTitle")} :code=${code} :script=${script}>
    <span slot="status" class="demo-state">{{ circleProgress }}%</span>
    <div slot="controls" style="display:grid;gap:12px">
      <div style="display:flex;gap:8px;flex-wrap:wrap">
        ...
        <elf-button size="sm" variant="tonal" @click=${toggleAuto}>{{ autoRunning ? t("stop") : t("auto") }}</elf-button>
      </div>
  ...
  <h2>${t("status")}</h2>
  <elf-playground :title=${t("statusTitle")} :code=${statusCode} :script=${script}>
    <div style="display:grid;gap:16px;width:100%;max-width:520px"><elf-progress percentage="28" status="warning" striped-flow></elf-progress><elf-progress percentage="64" status="exception"></elf-progress><elf-progress indeterminate duration="1"></elf-progress></div>
  </elf-playground>
`);
